...
const NGODashboard: React.FC<NGODashboardProps> = ({ user, data, isOnline }) => {
  ...
  useEffect(() => {
    ...
    if (!leafletMap.current) {
      ...
      // Use CartoDB Positron for a clean Light Theme
    ...
    if (zoneLayers.current) {
      ...
      riskAnalysis.forEach((analysis) => {
        ...
        // Calculate rough centroid for marker placement
        const latTotal = (coords as any[]).reduce((sum: number, p: any) => sum + p.lat, 0);
        const lngTotal = (coords as any[]).reduce((sum: number, p: any) => sum + p.lng, 0);
        const centroid = [latTotal / coords.length, lngTotal / coords.length] as L.LatLngExpression;
        ...
        polygon.bindTooltip(`
          <div class="text-center">
            <div class="font-black text-[10px] uppercase tracking-widest mb-1 text-slate-400">${analysis.zone}</div>
            <div class="text-xs font-bold text-white">${(analysis.prob * 100).toFixed(0)}% Risk</div>
          </div>
        `, {
          permanent: false,
          direction: 'center',
          className: 'bg-slate-900/90 border border-white/10 text-white rounded-lg px-3 py-2 shadow-2xl backdrop-blur-md'
        });

        polygon.addTo(zoneLayers.current!);

        // Add Pulsing Impact Marker for High Risk Zones
        if (analysis.reportCount > 0) {
          const pulseColorClass = analysis.priority === 'Critical' ? 'bg-red-500 box-shadow-red' :
            analysis.priority === 'High' ? 'bg-amber-500 box-shadow-amber' :
              'bg-blue-500';

          const iconHtml = `
             <div class="relative w-4 h-4 group cursor-pointer">
                ${analysis.priority === 'Critical' || analysis.priority === 'High' ?
              `<div class="absolute inset-0 rounded-full ${pulseColorClass} opacity-75 animate-ping"></div>` : ''}
                <div class="relative w-4 h-4 rounded-full ${pulseColorClass} border-2 border-[#0f172a] shadow-lg flex items-center justify-center">
                  <span class="text-[6px] font-black text-white">${analysis.reportCount}</span>
                </div>
             </div>
           `;
          ...
          const marker = L.marker(centroid, { icon: customIcon });
          marker.on('click', () => setSelectedZoneDetail(analysis));
          marker.addTo(zoneLayers.current!);
        }
      });
    ...
    // Map Click Listener for Zone Placement
    if (leafletMap.current) {
      leafletMap.current.off('click'); // Remove previous listeners
      leafletMap.current.on('click', async (e: L.LeafletMouseEvent) => {
        if (isPickingLocation && newZoneName) {
          const { lat, lng } = e.latlng;

          const newZoneConfig: any = {
            id: newZoneName,
            name: newZoneName,
            coordinates: [
              { lat: lat + 0.004, lng: lng }, // Top
              { lat: lat - 0.003, lng: lng + 0.004 }, // Bottom Right
              { lat: lat - 0.003, lng: lng - 0.004 }  // Bottom Left
            ]
          };

          try {
            await addZone(newZoneConfig);
            setNewZoneName('');
            setIsPickingLocation(false);
            alert(`Zone ${newZoneName} placed successfully!`);
          } catch (err: any) {
  ...
  }, [riskAnalysis, isPickingLocation, newZoneName]);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      {isPickingLocation && (
        <div className="fixed top-24 left-1/2 transform -translate-x-1/2 z-[50] bg-blue-600 text-white px-8 py-4 rounded-full shadow-2xl animate-bounce flex items-center space-x-4">
          <span className="font-black uppercase tracking-widest text-xs">Click Map to Place "{newZoneName}"</span>
          <button
            ...
            <div className="p-8 overflow-y-auto space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-end mb-2">
                  <button
                    onClick={async () => {
                      if (!confirm("This will move all zones to a grid layout. Continue?")) return;

                      const baseLat = 21.1966;
                      const baseLng = 92.1583;
                      const spacing = 0.012; // ~1.2km spacing
                      const cols = 3;

                      const sortedZones = [...data.zones].sort((a, b) => a.name.localeCompare(b.name));
                      ...
                      for (let i = 0; i < sortedZones.length; i++) {
                        ...
                        // Center the grid
                        const lat = baseLat - (row * spacing) + (spacing * 1.5);
                        const lng = baseLng + (col * spacing) - (spacing * 1.5);
                        ...
                        try {
                          await addZone({ ...sortedZones[i], coordinates: newCoords });
                        } catch (e) {
                          console.error("Failed to move zone", e);
                        }
                      }
                      alert("Zones redistributed.");
                    }}
                    className="text-[9px] bg-slate-700 hover:bg-slate-600 text-slate-300 px-3 py-1.5 rounded-lg font-black uppercase transition-all"
                  >
                    Auto-Arrange Layout
                  </button>
                </div>
